...
export default function Home() {
  return (
    <div className="relative z-10 min-h-screen flex flex-col items-center">
      ...
      {/* --- GRID DE TARJETAS (Gap reducido y max-width ajustado) --- */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-8">
          ...
          {listaRifas.map((rifa, index) => (
            ...
            <div key={rifa.id} className="flex flex-col gap-1.5 h-full">
              ...
              {/* LA TARJETA (Radios más pequeños y compacta) */}
              <div 
                className="group relative bg-slate-900/80 backdrop-blur-xl rounded-[1.75rem] overflow-hidden shadow-2xl transition-all duration-500 border border-slate-700/50 hover:border-blue-500/50 hover:shadow-[0_8px_30px_rgba(0,0,0,0.6)] hover:-translate-y-1.5 flex flex-col h-full"
                style={{ animationDelay: `${index * 100}ms` }} 
              >
                
                {/* Contenedor de Imagen (Altura reducida: h-52 md:h-60) */}
                <div className="relative h-52 md:h-60 overflow-hidden shrink-0">
                  ...
                  {/* PRECIO FLOTANTE (Más sutil) */}
                  <div className="absolute top-3 right-3 md:top-4 md:right-4 bg-slate-950/90 backdrop-blur-sm text-white px-3 py-1.5 md:px-4 md:py-1.5 rounded-xl font-black text-base md:text-lg shadow-xl border border-blue-500/30 group-hover:border-blue-500 transition-colors">
                ...
                {/* Contenido de la Tarjeta (Paddings reducidos: p-5) */}
                <div className="p-5 flex flex-col flex-grow relative z-10">
                  <h2 className="text-xl md:text-2xl font-black text-white uppercase italic leading-tight mb-2 group-hover:text-blue-400 transition-colors line-clamp-2">
                    {rifa.nombre}
                  </h2>
                  ...
                  <div className="space-y-2.5">
                    <Link 
                      ...
                      className="relative overflow-hidden block w-full text-center bg-blue-600 text-white py-3 md:py-3.5 rounded-xl font-black uppercase text-[9px] md:text-[10px] tracking-[0.2em] transition-all hover:bg-blue-500 hover:shadow-[0_0_20px_rgba(37,99,235,0.4)] active:scale-95 border border-transparent hover:border-blue-300/50"
                    >
                      COMPRAR BOLETOS 🎟️
                    </Link>
                    
                    <div className="flex justify-between items-center px-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                        <span className="text-[7px] text-slate-500 font-bold uppercase">Sorteo Verificado</span>
          ...
          {/* TARJETA "PRÓXIMAMENTE" (Proporcional a las nuevas) */}
